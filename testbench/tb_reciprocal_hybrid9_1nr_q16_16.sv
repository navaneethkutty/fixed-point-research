...
// ============================================================================
// Testbench for reciprocal_hybrid9_1nr_q16_16.
//
// Direct Q16.16 port of tb_reciprocal_hybrid9_1nr_q8_24. Methodology is
// UNCHANGED: same reset-adjacent valid_in stress case (DUT_WARMUP_CYCLES =
// STARTUP_MARGIN_CYCLES = 0), same event-counted-guard drop-by-design
// handling (9 pulses masked for DISCARD_TOKENS=8), same pre-guard
// token-index FIFO for golden alignment, same drain-based completion.
// Only format-specific constants change: file names, the Q16.16 <-> real
// scale factor (2^16 = 65536, was 2^24 = 16777216), and the reported
// quantization label.
// ============================================================================

module tb_reciprocal_hybrid9_1nr_q16_16;
    ...
    // Same regression case as the Q8.24 testbench; keep it this way.
    localparam int DUT_WARMUP_CYCLES = 0;
    localparam int STARTUP_MARGIN_CYCLES = 0;

    // Widened from 1000 so the run does not clip the tail of the sweep.
    // Retained as an upper bound for the watchdog; completion itself is
    // drain-based, not count-based.
    ...
    initial begin
        $readmemh("q16_16_input.mem", test_x);
        $readmemh("q16_16_expected.mem", test_golden);

        if (^test_x[0] === 1'bx)
            $fatal(1, "ERROR: q16_16_input.mem failed to load.");

        if (^test_golden[0] === 1'bx)
            $fatal(1, "ERROR: q16_16_expected.mem failed to load.");
    end
    ...
    reciprocal_hybrid9_1nr_q16_16 u_dut (
        .clk(clk),
        .rst_n(rst_n),
        .valid_in(valid_in),
        .x_in(x_in),
    ...
    // well past the first several real outputs (pipeline depth unchanged
    // from Q8.24, so the same window size is still sufficient).
    ...
    initial begin
        ...
        if (DEBUG_PROBE_CYCLES > 0) begin
            fd_probe = $fopen("nr_stage_debug_trace_q16_16.csv", "w");

            if (fd_probe == 0)
                $fatal(1, "ERROR: Could not open nr_stage_debug_trace_q16_16.csv");
    ...
    // (an event) rather than a specific cycle count.
    logic guard_check_done;

    // Q16.16 scale factor: 2^16 (was 2^24 for Q8.24).
    localparam real QFRAC_SCALE = 65536.0;
    ...
    initial begin
        ...
        fd_csv = $fopen("hybrid9_1nr_q16_16_results.csv", "w");
        fd_summary = $fopen("hybrid9_1nr_q16_16_summary.txt", "w");

        if (fd_csv == 0)
            $fatal(1, "ERROR: Could not open hybrid9_1nr_q16_16_results.csv");

        if (fd_summary == 0)
            $fatal(1, "ERROR: Could not open hybrid9_1nr_q16_16_summary.txt");
    ...
    always @(posedge clk) begin
        ...
        if (run_complete) begin
            ...
            $display("============================================================");
            $display(
                "reciprocal_hybrid9_1nr_q16_16 verification"
            );
            $display("============================================================");
            $display(
