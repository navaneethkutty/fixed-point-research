...
module tb_reciprocal_interp_q24_8;

    localparam int NVEC = 1_000_001;
    localparam real CLK_PERIOD_NS = 2.58;

    // Q24.8: 8 fractional bits -> 1 LSB = 1/256
    localparam real LSB_SCALE = 256.0;
    ...
    initial begin
        $readmemh("q24_8_input.mem", test_x);
        $readmemh("q24_8_expected.mem", test_golden);

        if (^test_x[0] === 1'bx)
            $fatal(1, "ERROR: q24_8_input.mem failed to load.");

        if (^test_golden[0] === 1'bx)
            $fatal(1, "ERROR: q24_8_expected.mem failed to load.");
    end
    ...
    initial begin
        ...
        fd_csv = $fopen("interp_lut_results_q24_8.csv", "w");
        fd_summary = $fopen("interp_lut_summary_q24_8.txt", "w");

        if (fd_csv == 0)
            $fatal(1, "ERROR: Could not open interp_lut_results_q24_8.csv");

        if (fd_summary == 0)
            $fatal(1, "ERROR: Could not open interp_lut_summary_q24_8.txt");
    ...
    always @(posedge clk) begin
        ...
        if (rst_n && valid_out) begin
            ...
            if (out_idx == NVEC - 1) begin
                ...
                // Q24.8 -> 8 fractional bits, so 1 LSB = 1/256
                // (was /16777216.0 for Q8.24's 24 fractional bits)
                mae_value =
                    mae_lsb / LSB_SCALE;

                $display("============================================================");
                $display("reciprocal_interp_q24_8 verification");
                $display("============================================================");
                $display("Test vectors        : %0d", NVEC);
                $display("Input range         : [0.5, 2.5]");
                $display("Quantization        : Q24.8");
                ...
                $fwrite(fd_summary,
                        "============================================================\n");
                $fwrite(fd_summary,
                        "reciprocal_interp_q24_8 verification\n");
                $fwrite(fd_summary,
                        "============================================================\n");
                $fwrite(fd_summary,
                        "Test vectors        : %0d\n", NVEC);
