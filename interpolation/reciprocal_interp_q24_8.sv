// ============================================================================
// reciprocal_interp_q24_8
//
// 6-bit LUT + linear interpolation reciprocal for x in [0.5, 2.5], Q24.8
//
// REV 7 + P1 COMPARATOR OPTIMIZATION  (converted from Q8.24 -> Q24.8)
//
// Format note:
//   Q8.24 had 24 fractional bits, so the offset x-X_LO spanned 25 bits
//   (IDX_BITS+SHIFT = 11+14 = 25). Q24.8 only has 8 fractional bits, so
//   the same offset only spans 9 bits (2.0 * 2^8 = 512). IDX_BITS+SHIFT
//   was therefore re-split to sum to 9 instead of 25 (6+3), with
//   FRAC_BITS=SHIFT=3 chosen so that N*2^FRAC_BITS = 64*8 = 512 lands
//   exactly on every representable input code (no gaps, no aliasing).
//   Every other pipeline stage operates on raw two's-complement integers
//   and is completely format-agnostic, so it is untouched.
//
// Pipeline:
...
module reciprocal_interp_q24_8 #(
    parameter int IDX_BITS        = 6,
    parameter int SHIFT           = 3,
    parameter int FRAC_BITS       = 3,
    parameter int SLOPE_FRAC_BITS = 21   // NOTE: declared for documentation
                                          // parity with the Q8.24 source;
                                          // the actual hardware shift is
                                          // the hardcoded ">>21" below (see
                                          // P8 comment). Keep in sync if
                                          // you ever change that literal.
) (
    input  logic        clk,
    input  logic        rst_n,
    ...
    // 0.5 and 2.5 in Q24.8 (24 integer bits, 8 fractional bits)
    localparam logic [31:0] X_LO = 32'h0000_0080;   // 0.5 * 2^8
    localparam logic [31:0] X_HI = 32'h0000_0280;   // 2.5 * 2^8
    ...
    // X_HI = 2.5 = 0x00000280.
    //
    // Q24.8 layout: x_in[31:8] is the 24-bit integer field,
    // x_in[7:0] is the 8-bit fractional field.
    //
    // Therefore:
    //   integer field > 2   -> above
    //   integer field < 2   -> not above
    //   integer field == 2  -> compare fractional field > 0x80 (0.5)
    //
    ...
    always_comb begin
        if (x_in[31:8] > 24'd2)
            above_comb_p0 = 1'b1;
        else if (x_in[31:8] < 24'd2)
            above_comb_p0 = 1'b0;
        else
            above_comb_p0 = (x_in[7:0] > 8'h80);
    end
    ...
    xpm_memory_sprom #(
        ...
        .MEMORY_INIT_FILE   ("interp_seed_q24_8.mem"),
    ...
    xpm_memory_sprom #(
        .ADDR_WIDTH_A       (IDX_BITS),
        .MEMORY_INIT_FILE   ("interp_slope_q24_8.mem"),
        .MEMORY_PRIMITIVE   ("block"),
    ...
    // Zero-pad frac_r4 up to the 18-bit B port. Generalized from the
    // Q8.24 source's hardcoded "{4'b0, frac_r4}" (which assumed
    // FRAC_BITS=14) so it stays correct for any FRAC_BITS <= 18.
    ...
    // NOTE: shift amount is intentionally left at 21 (unchanged from the
    // Q8.24 source, see SLOPE_FRAC_BITS comment above). The Q24.8 LUT
    // generation script scales its slope values against this same
    // constant, so this line does not need to change.
