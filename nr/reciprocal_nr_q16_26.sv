...
// ============================================================================
// pipelined_mult32x32_signed
//
// Generic pipelined 32x32 -> 64 signed multiply. Format-agnostic: it just
// produces the exact 64-bit two's-complement product of its two 32-bit
// inputs, built from four 17x17 partial products.
//
// ROUND_BIAS is NOT format-agnostic, even though this module otherwise is.
// It is added into the low partial product (pll) before the four partials
// are summed, i.e. it lands at bit 0 of the full 64-bit product. Whoever
// calls this module and later truncates the 64-bit product down to a
// Q(F) result by right-shifting F bits (slicing p[31+F : F]) needs
// ROUND_BIAS = 2^(F-1) -- exactly half of that truncation quantum -- to
// get correct round-to-nearest. Get this wrong and the bias lands at the
// wrong bit weight relative to the truncation point: too small to matter,
// or (worse, as with a straight Q8.24 -> Q16.16 port) many times too
// large, silently corrupting every result with no timing violation and
// no simulation error to flag it.
//
// Default here (2^23) matches the original Q8.24 caller
// (reciprocal_nr_q8_24, F=24, truncates at bit 24). Any new caller with a
// different F MUST override this explicitly -- see
// reciprocal_nr_q16_16's instantiations below (F=16 -> 2^15 = 32768).
// ============================================================================
module pipelined_mult32x32_signed #(
    parameter logic signed [47:0] ROUND_BIAS = 48'sd8388608
) (
    input logic clk,
    input logic rst_n,
    input logic in_valid,
    ...
    // ever changes -- reciprocal_nr's alignment shift registers are
    ...
    // history below). Format-agnostic: unaffected by the Q8.24 -> Q16.16
    // port, since ROUND_BIAS only changes a DSP addend value, not the
    // pipeline's register count.
    //
...
// ============================================================================
// reciprocal_nr_q16_16
//
// Converted from reciprocal_nr_q8_24. Two Newton-Raphson refinement
// iterations (y_{n+1} = y_n * (2 - x*y_n)) seeded from a small BRAM LUT.
//
// Format change notes (Q8.24 -> Q16.16), F = 16 throughout:
//   - X_LO / X_HI / TWO re-encoded for 16 integer / 16 fractional bits.
//   - offset_r1 narrows from 26 -> 18 bits (max offset 2.0 needs F+2 bits),
//     same reasoning as the LUT+interpolation conversion.
//   - SHIFT drops from 17 -> 9 (= F+1-IDX_BITS, IDX_BITS held at 8).
//   - Every 32x32 product requantization slice moves from
//     mult_p[55:24] (right-shift-by-24) to mult_p[47:16]
//     (right-shift-by-16), since Q(F)*Q(F) = Q(2F) and getting back to
//     Q(F) means discarding the low F bits of the 64-bit product.
//   - ROUND_BIAS passed into pipelined_mult32x32_signed changes from
//     2^23 to 2^15 at all four instantiations below -- see the long
//     comment on pipelined_mult32x32_signed above. This is NOT optional:
//     reusing the Q8.24 default here would silently corrupt every NR
//     iteration with no timing or simulation error to flag it.
//   - IDX_BITS stays at 8 and MULT32_LATENCY (L=14) is unchanged, since
//     neither depends on the Q format.
//   - nr_seed.mem MUST be regenerated with reciprocal seed values
//     quantized as Q16.16 (still 256 entries, IDX_BITS unchanged).
// ============================================================================
module reciprocal_nr_q16_16 #(
    ...
    localparam logic signed [31:0] TWO = 32'sh0002_0000;    // 2.0 in Q16.16
    localparam int N = (1 << IDX_BITS);

    // Half the Q16.16 truncation quantum (2^16), for round-to-nearest
    // when requantizing 64-bit products back down to Q16.16. See the
    // ROUND_BIAS comment on pipelined_mult32x32_signed.
    localparam logic signed [47:0] NR_ROUND_BIAS = 48'sd32768;

    // Must always equal pipelined_mult32x32_signed::MULT32_LATENCY.
    // Format-agnostic -- unchanged from the Q8.24 version.
    localparam int L = 14;
    ...
    initial begin
        $readmemh("nr_seed_q16_16_256.mem", seed_mem);
        // synthesis translate_off
        if (^seed_mem[0] === 1'bx)
            $fatal(1, "ERROR: nr_seed_q16_16_256.mem failed to load.");
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            p1_round_r <= $signed(mult1a_p[47:16]);
            v_p1 <= mult1a_v;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            y1_round_r <= $signed(mult1b_p[47:16]);
            v_y1 <= mult1b_v;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            p2_round_r <= $signed(mult2a_p[47:16]);
            v_p2 <= mult2a_v;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            y2_round_r <= $signed(mult2b_p[47:16]);
            v_y2 <= mult2b_v;
        end
    end
